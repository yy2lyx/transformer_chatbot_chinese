...
def encode_reply(words, word_map,max_len):
    ...
    if len(words) > max_len:
        enc_c = [word_map['<start>']] + [word_map.get(word, word_map['<unk>']) for word in words][:max_len] + [word_map['<end>']]
    return enc_c
...
if __name__ == '__main__':
    ...
    # 对语料集进行处理（分词 + 填充标志符 + 限制长度）
    # 分词 == 生成word2int
    seg = pkuseg.pkuseg(model_name="web", user_dict="default")
    new_corpus = []
    word_freq = Counter()
    for i in range(len(corpus)):
        start_words = seg.cut(corpus[i][0])
        end_words = seg.cut(corpus[i][1])
